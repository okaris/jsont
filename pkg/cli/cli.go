// Package cli provides the top-level jt command as an importable function,
// so other Go programs (e.g. belt) can embed jt without shelling out.
package cli
...
// Version can be set by the embedding binary.
...
// Run executes the jt CLI with the given args (os.Args[1:] equivalent).
...
func runSchema(objects []any, f flags, outOpts output.Opts) error {
	...
	for _, sf := range filtered {
		...
		if len(sf.Types) > 1 {
			...
			typeStr = strings.Join(filteredTypes, "|")
		}
...
// PrintHelp outputs the jt help text.
func PrintHelp() {
